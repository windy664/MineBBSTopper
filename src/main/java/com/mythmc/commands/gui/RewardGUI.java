...
import com.mythmc.impl.database.DatabaseCallback;
...
public class RewardGUI implements Listener {
    ...
    public static void openMenu(Player player) {
        // 创建一个新的菜单，标题可能会根据 PlaceholderHook 进行替换
        ...
        for (String key : menuItems.getKeys(false)) {
            ...
            if (label != 100) {
                if (rewardData[label - 1].equals("0")) {
                    // 未领取状态，创建一个普通物品堆
                    itemStack = ItemUtil.handleItemStack(player, materialName, materialAmount);
                } else {
                    // 已领取状态，创建一个标记为已领取的物品堆
                    itemStack = new ItemStack(XMaterial.valueOf(GUIFile.rewardMenuClaimedMat).parseMaterial());
                }
            } else {
                // 标签为100，创建一个普通物品堆
                itemStack = ItemUtil.handleItemStack(player, materialName, materialAmount);
            ...
            for (int i = 0; i < size; i++) {
                String line = ColorUtil.colorize(lore.get(i));
                // 替换描述中的状态占位符
                if (line.contains("%status%")) {
                    int require = itemSection.getInt("require");
                    if (rewardData[label - 1].equals("1")) {
                        line = line.replace("%status%", GUIFile.rewardMenuStatusClaim);
                    } else if (need >= require) {
                        line = line.replace("%status%", GUIFile.rewardMenuStatusYes.replace("%count%", String.valueOf(require)).replace("%current%", String.valueOf(need)));
                    } else {
                        line = line.replace("%status%", GUIFile.rewardMenuStatusNo.replace("%count%", String.valueOf(require)).replace("%current%", String.valueOf(need)));
                    }
                }
                // 更新列表中的该行
                lore.set(i, line);
            }
            // 如果使用 PlaceholderHook，替换描述中的占位符
        ...
        // player.playSound(player.getLocation(), Sound.valueOf(GUIFile.rewardMenuOpenSound), 1, 1); Bukkit音效
        // XSound.matchXSound(GUIFile.rewardMenuOpenSound).get().play(player); XSound音效
    }
    ...
    private void countPlayerRecordsAsync(String playerName, DatabaseCallback<Integer> callback) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                Bukkit.getScheduler().runTask(MineBBSTopper.instance, () -> callback.onQueryDone(count));
            }
