        ...
        if (player != null) {
            ...
            if (TimeUtil.isCurrentHourWithinSpan()) {
                ...
                if (cooldownTimestamp >= currentTime) {
                    ...
                    Debugger.logger("玩家 " + playerName + " 尝试获取奖励失败。原因：顶贴冷却中");
                    return false; // 结束方法，返回失败
                }
                ...
                if (ConfigFile.enableLimit) {
                    ...
                    if (todayAmount > ConfigFile.limitTimes - 1) {
                        ...
                        Debugger.logger("玩家 " + playerName + " 尝试获取奖励失败。原因：超过当日限制次数：" + todayAmount);
                        return false; // 结束方法，返回失败
                    }
                }
                ...
                ParserManagerFactory.getParserManager().fetchTimeElementsAsync(status -> {
                    ...
                    if (status != null && !status.isEmpty()) {
                        ...
                        // 如果获取到的时间元素为 "error"，表示网络连接失败
                        if (status.equals("error")) {
                            Debugger.logger("玩家 " + playerName + " 尝试获取奖励失败。原因：网页连接失败");
                            // 发送消息给玩家，告知网络错误
                    ...
                    Debugger.logger("为玩家 " + playerName + " 检测顶贴状态成功，进程已释放");
                });
            } else {
                // 如果当前时间不在可顶贴时间段内
                Debugger.logger("玩家 " + playerName + " 尝试获取奖励失败。原因：未在可顶贴时间段内");
                // 发送消息给玩家，告知未在可领取时间段内
